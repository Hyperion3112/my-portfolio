    ...
    <section id="experience" className="py-24">
      ...
      <div className="flex flex-col gap-12">
        {experiences.map((exp, idx) => (
          <motion.div
            key={exp.title}
            className="rounded-2xl p-0 flex flex-row items-center max-w-3xl transition-all duration-200 bg-transparent border-transparent shadow-none hover:bg-[#112240] hover:border-[#64ffda] hover:shadow-[0_4px_32px_0_rgba(100,255,218,0.10)] border group"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.6 }}
            variants={experienceVariants}
          >
            {/* Left: Years */}
            <div className="min-w-[110px] md:min-w-[140px] px-8 py-4 text-right">
              <span className="text-[#8892b0] font-mono text-xs tracking-wide whitespace-nowrap">{exp.period}</span>
            </div>
            {/* Right: Content */}
            <div className="flex-1 flex flex-col -ml-4 px-8 py-4 md:pr-8">
